...
function saveMessage(code, from, to, room, type, uuid, message, callback) {
    var cfg = {
    ...
    console.log(">>>>>>>par: code="+code+", from="+from+", to="+to+", room="+room+", type="+type+", uuid="+uuid+", message="+message);

    sock.ccps(cfg, par, cal);
}

function authMessage(name, pass, time, cal) {
    var cfg = {
    ...
    var par = {
        clzz: "['java.lang.String','java.lang.String','java.lang.Long']",
        json: [name, pass, time]
    };  
/*
...
module.exports = function(server,session)  {
    ...
    io.on('connection', function (socket) {
        ...
        socket.on(conf.ccps.auth.auth, function (name, pass, time){
            //session.user = {userId:userId, addr};
            authMessage(name, pass, time, function(err, obj){
                var code = chatCode.SUCC;
                if(err) {
                    code = chatCode.LOGINER;   
                }

                socket.emit(conf.ccps.auth.auth, code, chatType.ONLINE, "", socket.id, "", obj.userId); 
            });
        });
